...
if (defined('ADMIN_ALLOWED') == true) {
	...
	$queryString = pro_db_query("select mem.memberID, mem.memberName, mem.memberImage, mem.adminType, mem.memberEmail, mem.memberMobile, mem.bloodGroup,
										...
										from blockFloorOfficeMapping bfom
	...
	while ($res = pro_db_fetch_array($queryString)) {
		$query = pro_db_query(
			"SELECT desgMap.memberID, desg.designationTitle as memberInCommittee FROM designationMemberMapping desgMap
			join designationMaster desg on desgMap.designationID = desg.designationID 
			where desgMap.complexID = " . $_SESSION['complexID'] . " and desgMap.memberID = " . $res['memberID'] . " 
			group by desg.designationID"
		);
		$arrComm = array();
		while ($resComm = pro_db_fetch_array($query)) {
			$arrComm[] = $resComm["memberInCommittee"];
		}
		...
		if ($isAdmin) {
			...
			$memberName = '<td><a href="index.php?controller=complexmasters&action=membermaster&subaction=makeAdminForm&memberID=' . $res['memberID'] . '" title="Make Admin"><i class="far fa-user-circle icon-sm text-info"></i></a>' . '&nbsp&nbsp' . ucfirst($res['memberName']) . '</td>';
		}
		...
		if (!empty($arrComm)) {
			$committeeTitle = '<td>' . nl2br(implode("\n\n", $arrComm)) . '</td>';
		} else {
			$committeeTitle = '<td></td>';
		}
		...
		if (($_SESSION['complexID'] == 1001) || ($_SESSION['complexID'] == 1002)) {
			$Action = '<td><a href="index.php?controller=complexmasters&action=membermaster&subaction=editForm&memberID=' . $res['memberID'] . '" title="Edit"><i class="fe-edit text-info"></i></a>&nbsp;&nbsp;
							<a href="index.php?controller=complexmasters&action=membermaster&subaction=delete&memberID=' . $res['memberID'] . '" title="Delete"><i class="fe-trash-2 text-danger"></i></a>
						</td>';
		} else {
			$Action = '<td><a href="index.php?controller=complexmasters&action=membermaster&subaction=editForm&memberID=' . $res['memberID'] . '" title="Edit"><i class="fe-edit text-info"></i></a></td>';
		}

		$office = '<td>' . $res['officeName'] . '</td>';
		$result['aaData'][] = array("$office", "$flatNumber", "$memberImage", "$memberName", "$memberMobile", "$professionTitle", "$bloodGroup", "$committeeTitle", "$isAppUser", "$memberStatus", "$Action");
	}
	// End While Loop
