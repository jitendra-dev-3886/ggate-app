...
?>
						...
						<form role="form" class="form-horizontal" action="<?php echo $this->addformaction; ?>" method="post" enctype="multipart/form-data">
							<div class="row">
								<div class="form-group col-sm-3">
									<label>Title:</label>
									<input type="text" name="faqType" class="form-control" placeholder="" required>
								</div>
								<div class="form-group col-sm-3">
									<label>Sort Order:</label>
									<input name="sortorder" class="form-control">
								</div>
								<div class="form-group col-sm-3">
									<label>Status:</label>
	...
	{
		$sql = "select * from faqMaster where faqTypeID = " . (int)$_REQUEST['faqTypeID'];
					...
					<div class="card-body">
						<form role="form" class="form-horizontal" action="<?php echo $this->editformaction; ?>" method="post" enctype="multipart/form-data">
							<div class="row">
								<div class="form-group col-sm-3">
									<label>Title:</label>
									<input type="text" name="faqType" class="form-control" value="<?PHP echo $rs['faqType']; ?>" placeholder="" required>
								</div>
								<div class="form-group col-sm-3">
									<label>Sort Order:</label>
							...
							<div class="row">
								<div class="form-group col-sm-12">
									<input type="hidden" name="faqTypeID" value="<?php echo (int)$rs['faqTypeID']; ?>">
	...
	public function add()
	{
		...
		if (pro_db_perform('faqMaster', $formdata)) {
			$faqID = pro_db_insert_id();
			$msg = '<p class="bg-success p-3">FAQ Type Detail is saved successfully...</p>';
		} else {
			$msg = '<p class="bg-danger p-3">FAQ Type Detail is not saved!!!</p>';
		}
	...
	public function edit()
	{
		...
		if (pro_db_perform('faqMaster', $formdata, 'update', $whr)) {
			$msg = '<p class="bg-success p-3">FAQ Type Detail is saved successfully...</p>';
		} else {
			$msg = '<p class="bg-danger p-3">FAQ Type Detail is not saved!!!</p>';
		}
	...
	{
		global $frmMsgDialog;
		$delsql = "Delete from faqMaster where faqTypeID = " . (int)$_REQUEST['faqTypeID'];
		if (pro_db_query($delsql)) {
			$msg = '<p class="bg-success p-3">FAQ Type Detail is deleted...</p>';
		} else {
			$msg = '<p class="bg-danger p-3">FAQ Type Detail is not deleted!!!</p>';
		}
		$rUrl = $this->redirectUrl . "&subaction=listData";
		echo sprintf($frmMsgDialog, $rUrl, $msg);
	}
	...
	?>
				...
				<div class="card">
					<div class="card-body">
						<div class="table-responsive">
							<table cellpadding="1" cellspacing="2" border="0" class="table table-striped table-bordered dataTable" id="faqTypeList" width="100%">
								<thead>
									<tr>
		...
		<script>
			$(document).ready(function() {
				var listURL = 'helperfunc/faqTypeList.php';
